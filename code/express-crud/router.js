 ...
 * @LastEditTime: 2020-07-11 22:17:26
 * @LastEditors: Please set LastEditors
 * @Description: In User Settings Edit
 * @FilePath: \code\express-crud\router.js
 */
/**
 * router.js路由模块
 * 职责：
...
router.post('/students/new', function(req, res) {
    ...
    console.log(newStudent)
        // 2.将数据放在文件中
    Student.save(newStudent, function(err, data) {
        if (err) {
            return res.status(500).send("server error");
        };
        // 3.重定向
...
router.post('/students/edit', function(req, res) {
    ...
    // 2.修改数组中的数据，splice方法
    Student.update(editStudent, function(err, data) {
        if (err) {
...
router.get('/students/delete', function(req, res) {
    ...
    let id = req.query.id;
    // 2.数据操作，删除id
    Student.delete(id, function(err, data) {
        if (err) {
