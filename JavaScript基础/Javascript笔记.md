...
意思是：“可达”值 是那些以某些方式访问 或 可用 的值。它们一定是存储在内存中的
...
1. 固有的可达值的基本集合，这些值不能被释放
   ...
   这些值被称作 “根”，意思是一定不能被释放，只有引用（依赖）它们的，没有它们依赖别人的

2. 如果说一个值可以通过引用 或者引用链 从根访问 任何其他值，则认为改值是可达的

   比方说：如果全局变量中有一个对象，并且该对象有一个属性引用了另一个对象，则该对象被认为是可达的，而且它引用的内容也是可达的

注意点：

- 对外引用不重要，只有传入引用才可以使对象可达。也就是说依赖别人不可以，必须得被依赖，才可以算可达
...
- 垃圾收集器找到所有的根，并标记它们；
- 然后它遍历并标记来自它们的所有引用。（找到依赖它们的，第一层依赖）
- 然后它遍历标记的对象 并标记 **他们的** 引用。（第二层依赖，依赖的依赖）所有被遍历到的对象都会被记住（直到所有的依赖都被找完，像树一样），以免将来再次遍历到同一个对象
